...
function App() {
  ...
  const createUser = () => {
    axios.post(` https://mysterious-plateau-76470.herokuapp.com/${username}`);
  }

  //update month
  const getMon = () => {
    axios.get(` https://mysterious-plateau-76470.herokuapp.com/`).then(res => setMonth(res.data.month));
  }

  useEffect(() => getMon());

  const getYr = () => {
    axios.get(` https://mysterious-plateau-76470.herokuapp.com/`).then(res => setYear(res.data.year));
  }
  ...
  const getLimit = () => {
    axios.get(` https://mysterious-plateau-76470.herokuapp.com/${username}`).then(res => setLimit(res.data.limit));
  }
  ...
  const getBalance = () => {
    axios.get(` https://mysterious-plateau-76470.herokuapp.com/${username}`).then(res => setBalance(res.data.balance));
  }
  ...
  const updateLimit = (lim) => {
    axios.post(` https://mysterious-plateau-76470.herokuapp.com/${username}`, { lim: lim }).then(
      res => setLimit(lim)
    );
  }
  ...
  const getYearActions = () => {
    axios.get(` https://mysterious-plateau-76470.herokuapp.com/${username}?year=${year}`).then(
  ...
  const makeTransaction = (desc, amnt, mon, day, yr, cat) => {
    axios.post(` https://mysterious-plateau-76470.herokuapp.com/${cat}?month=${month}&year=${year}`,
