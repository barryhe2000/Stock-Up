...
function App() {
  ...
  const createUser = () => {
    axios.post(` https://mysterious-plateau-76470.herokuapp.com/action/${username}`);
  }

  //update month
  const getMon = () => {
    axios.get(` https://mysterious-plateau-76470.herokuapp.com/month/`).then(res => setMonth(res.data.month));
  }

  useEffect(() => getMon());

  const getYr = () => {
    axios.get(` https://mysterious-plateau-76470.herokuapp.com/year/`).then(res => setYear(res.data.year));
  }
  ...
  const getLimit = () => {
    axios.get(` https://mysterious-plateau-76470.herokuapp.com/getlimit/${username}`).then(res => setLimit(res.data.limit));
  }
  ...
  const getBalance = () => {
    axios.get(` https://mysterious-plateau-76470.herokuapp.com/getbalance/${username}`).then(res => setBalance(res.data.balance));
  }
  ...
  const updateLimit = (lim) => {
    axios.post(` https://mysterious-plateau-76470.herokuapp.com/updatelimit/${username}`, { lim: lim }).then(
      res => setLimit(lim)
    );
  }
  ...
  const getYearActions = () => {
    axios.get(` https://mysterious-plateau-76470.herokuapp.com/getallactions/${username}?year=${year}`).then(
  ...
  const makeTransaction = (desc, amnt, mon, day, yr, cat) => {
    axios.post(` https://mysterious-plateau-76470.herokuapp.com/maketransaction/${cat}?month=${month}&year=${year}`,
